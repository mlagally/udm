...
var Amqp = (function (_super) {
        ...
        _this._fsm = new machina.Fsm({
            ...
            states: {
                disconnected: {
                    ...
                    send: function (message, deviceEndpoint, callback) {
                        ...
                        _this._fsm.handle('connect', function (err) {
                            if (err) {
                                ...
                                _this._fsm.handle('send', message, deviceEndpoint, callback);
                            }
                        });
                    },
                ...
                connecting: {
                    _onEnter: function (callback) {
                        var uri = _this._getConnectionUri();
                        _this._amqp.connect(uri, undefined, function (err, result) {
                            if (err) {
    ...
    Amqp.prototype.connect = function (done) {
        this._fsm.handle('connect', function (err) {
            if (err) {
                done(amqp_service_errors_js_1.translateError('AMQP Transport: Could not connect', err));
            }
            else {
                done(null, new azure_iot_common_1.results.Connected());
            }
        });
    };
    /**
    ...
    Amqp.prototype.updateSharedAccessSignature = function (sharedAccessSignature, callback) {
        ...
        this._fsm.handle('updateSharedAccessSignature', sharedAccessSignature, function (err) {
            if (err) {
                /*Codes_SRS_NODE_IOTHUB_SERVICE_AMQP_16_017: [** All asynchronous instance methods shall call the `done` callback with a single parameter that is derived from the standard Javascript `Error` object if the operation failed.]*/
                callback(err);
            }
